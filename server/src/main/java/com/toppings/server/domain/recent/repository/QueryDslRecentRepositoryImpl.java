...
public class QueryDslRecentRepositoryImpl implements QueryDslRecentRepository {
	...
	public List<RecentResponse> findRecents(
		...
		return queryFactory.select(
			Projections.fields(RecentResponse.class, recent.id, recent.keyword, recent.content, recent.category))
			.from(recent)
	...
	public Optional<Recent> findRecentByRecentRequest(
		...
		return Optional.ofNullable(queryFactory.select(recent)
			.from(recent)
			.where(likeKeyword(request), likeContent(request), eqUserId(user.getId()))
			.fetchOne());
	}
	...
	private BooleanExpression likeContent(RecentRequest request) {
		return request != null && hasText(request.getContent()) ? recent.content.contains(request.getContent()) : null;
	}

	private BooleanExpression likeKeyword(RecentRequest request) {
		return request != null && hasText(request.getKeyword()) ? recent.keyword.contains(request.getKeyword()) : null;
	}
}
